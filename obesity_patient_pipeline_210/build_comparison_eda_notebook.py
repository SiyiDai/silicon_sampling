...
DEFAULT_RUN_DIR = PIPELINE_DIR / "data" / "obesity_questionnaire_210_persona_full_comparison"
...
def find_selected_sample_path(run_dir: Path) -> Path:
    for name in ["selected_personas.csv", "selected_210_personas.csv", "selected_10_personas.csv"]:
        path = run_dir / name
        if path.exists():
            return path
    matches = sorted(run_dir.glob("selected_*_personas.csv"))
    if matches:
        return matches[0]
    raise FileNotFoundError(f"No selected persona CSV found in {run_dir}")
...
def build_comparison_tables(run_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    sample_path = find_selected_sample_path(run_dir)
    ...
    output_summary = {
        ...
        "run_dir": str(run_dir),
        "selected_sample": str(find_selected_sample_path(run_dir)),
        "variants": "both",
...
def build_notebook(
    ...
    cells = [
        md_cell(
            cjk_style_block()
            + "# 210人persona问卷作答：有/无背景信息影响分析\n\n"
            "本 notebook 对完整210位CFPS persona进行比较：一版不提供额外竞品/产品背景信息，一版提供中文讲解式背景信息。"
        ),
        md_cell(
            "## 1. 结论导读\n\n"
        ...
        md_cell("## 5. 210位persona人口学与健康画像\n\n" + df_to_markdown(demographic_summary, max_rows=30)),
